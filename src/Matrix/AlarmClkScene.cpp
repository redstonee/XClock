...
bool AlarmClkLayer::initLayer()
{
    ...
    {
        currentAlarmIndex = 0;
        AlarmTime = getAlarmClock(currentAlarmIndex);
    }
    else
    {
        AlarmTime.hour = 0;
...
void AlarmClkLayer::AlarmUpdate(float dt)
{
    ...
    // Serial.printf("Alarm Clk sts:%d alarm idx:%d week idx:%d time: %d:%d:%x \n",enAlarmState,currentAlarmIndex,SettingWeekIdx,AlarmTime.hour,AlarmTime.minute,AlarmTime.week);
}
...
void AlarmClkLayer::StateDisShow(void)
{
    ...
    if (oldAlarmidx != currentAlarmIndex)
    {
        if (oldAlarmidx < currentAlarmIndex)
        {
...
void AlarmClkLayer::StateDisHandle(tenKeyID keyID, tenKeyEventType eventID)
{
    if (enKey_OK == keyID)
    {
        switch (eventID)
        {
        case enKey_ShortPress:
            AlarmTime.isActive = !AlarmTime.isActive;
            boSetAlarmClk(currentAlarmIndex, &AlarmTime);
            break;
    ...
    else if (enKey_Left == keyID)
    {
        switch (eventID)
        {
        case enKey_ShortPress: // previous alarm clock
            if (currentAlarmIndex-- <= 0)
            {
                currentAlarmIndex = numberOfAlarms - 1;
            }
            AlarmTime = getAlarmClock(currentAlarmIndex);
            break;
    ...
    else if (enKey_Right == keyID)
    {
        switch (eventID)
        {
        case enKey_ShortPress: // next alarm clock
            if (currentAlarmIndex++ >= numberOfAlarms)
            {
                currentAlarmIndex = 0;
            }
            AlarmTime = getAlarmClock(currentAlarmIndex);
        ...
        case enKey_LongPressStart:
            if (numberOfAlarms > 1)
            {
                if (boDelAlarmClk(currentAlarmIndex))
                {
                    currentAlarmIndex--;
                    numberOfAlarms = getAlarmClockCount();
                    AlarmTime = getAlarmClock(currentAlarmIndex);
...
void AlarmClkLayer::StateSetMinHandle(int8_t keyID, int8_t eventID)
{
    if (enKey_OK == keyID)
    {
        if (enKey_ShortPress == eventID) // enter hour setting state
        {
...
void AlarmClkLayer::StateSetHourHandle(int8_t keyID, int8_t eventID)
{
    if (enKey_OK == keyID)
    {
        if (enKey_ShortPress == eventID) // enter week setting state
        {
...
void AlarmClkLayer::StateSetWeekHandle(int8_t keyID, int8_t eventID)
{
    if (enKey_OK == keyID)
    {
        if (enKey_ShortPress == eventID) // current day need enable
        {
        ...
        else if (enKey_LongPressStart == eventID) // finish this alarm setting go back to display state
        {
            boSetAlarmClk(currentAlarmIndex, &AlarmTime);
            SettingWeekIdx = 0;
...
void AlarmClkLayer::AlarmStateMachine(int8_t keyID, int8_t eventID)
{
    ...
    {
    case State_AlarmDis:
        StateDisHandle(keyID, eventID);
        break;
    case State_AlarmSetMin:
        StateSetMinHandle(keyID, eventID);
        break;
    case State_AlarmSetHour:
        StateSetHourHandle(keyID, eventID);
        break;
    case State_AlarmSetWeek:
        StateSetWeekHandle(keyID, eventID);
        break;
    default:
        break;
    }
